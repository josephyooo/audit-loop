...
2. **Deadlock recovery:** Before grouping new issues, check for open `codex-audit` PRs without reviews:
   ...
   ```bash
   tmux send-keys -t codex "/audit AUDIT: review PR #<number>" Enter
   ```
...
4. **Parse review comments into explicit tasks before editing.**
   ...
   ```bash
   tmux send-keys -t codex "/audit AUDIT: review PR #N" Enter
   ```
   Then stop and wait.
...
```bash
tmux send-keys -t codex "/audit AUDIT: complete" Enter
```
